...
| 阶段 | 内容 | 状态 |
|---|---|---|
| **P1.0** | 基础架子（FastAPI + DB + 单条 grade 接口 + 写死 rubric） | TODO |
| **P1.1** | OCR 流水线（PDF → 题目库），至少入库 30 道题 | TODO |
| **P1.2** | System Prompt v2 + Few-shot + 重试/校验 | TODO |
| **P1.3** | 评分缓存 + 异步化 | TODO |
| **P1.4** | 评测集 + 一致性报告 | TODO |
| **P2 (后续)** | 引入性能维度 + Docker 沙箱 + 多语言支持 | 规划中 |
